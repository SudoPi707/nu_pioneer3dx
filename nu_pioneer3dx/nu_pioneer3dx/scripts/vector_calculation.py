...
class LinearVector():
    ...
    def __init__(self):
        ...
        self.min_turn_speed = 0.05  # Turn is the speed of x according to z
        ...
        self.distance_torelance = 0.4
        self.yaw_torelance = math.pi / 24
    ...
    def order_robot(self, magnitude, goal_magnitude, final_yaw):
        ...
        if goal_magnitude > self.distance_torelance:
            ...
            xy_sigmoid = 1 / \
                (1 + math.exp(-(magnitude / self.speed_rate - self.delay)))
            rot_sigmoid = 1 / \
                (1 + math.exp(-(abs(final_yaw) / self.rot_rate - self.delay)))
            ...
            if abs(final_yaw) > self.yaw_torelance:  # If yaw is greater than yaw torelance
                ...
                if abs(final_yaw) > math.pi / 4:  # If robot turns its back to the targete
                    vel_x = (self.max_xy_speed * turn_sigmoid) + \
                        self.min_turn_speed

                else:
                    vel_x = self.max_xy_speed * xy_sigmoid
